  ...
  static async requestBooking(req, res) {
    try {
      ...
      if (user_type !== 'student') {
        return res.status(403).json({ success: false, message: 'Only students can make booking requests' });
      }

      const { room_preference, start_date, end_date } = req.body;
      if (!room_preference || !start_date || !end_date) {
        return res.status(400).json({ success: false, message: 'Room preference, start date, and end date are required' });
      }

      if (!req.file) {
        return res.status(400).json({ success: false, message: 'Admission letter is required' });
      }
      ...
      if (availableBeds.length === 0) {
        return res.status(400).json({ success: false, message: 'No available beds found matching your preferences' });
      }
